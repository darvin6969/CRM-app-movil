...
export default function ProfileScreen() {
    ...
    const handleLogout = async () => {
        ...
        try {
            ...
            // CLEAR LOCAL STATE
            setCustomer(null);
            // FORCE EXIT TO UNAMBIGUOUS LOGIN
            setTimeout(() => {
    ...
    const copyReferral = async () => {
        if (customer?.referralCode) {
            // Using placeholder copy action as full clipboard requires setup, but UI responds
    ...
    if (!customer) {
        return (
            <SafeAreaView className="flex-1 bg-slate-50 dark:bg-black items-center justify-center p-6">
                <Text className="text-xl font-bold text-slate-900 dark:text-white">Perfil no encontrado</Text>
                <TouchableOpacity 
                    onPress={() => router.replace('/login' as any)}
                    className="bg-primary/20 p-4 rounded-xl mt-4"
                >
                    <Text className="text-primary font-bold">¡FORZAR SALIDA AL LOGIN!</Text>
                </TouchableOpacity>
            </SafeAreaView>
        );
    }
                ...
                >
                    ...
                    {customer.referralCode && (
                        <LinearGradient
                            ...
                            className="rounded-[32px] p-8 mb-10 flex-row items-center justify-between shadow-xl shadow-primary/40"
                        >
                            <View className="flex-1">
                                <Text className="text-white/70 text-xs mb-1 font-black uppercase tracking-widest">Invita y Gana</Text>
                                <Text className="text-white font-black text-2xl tracking-tighter">{customer.referralCode}</Text>
                            </View>
                            <TouchableOpacity onPress={copyReferral} className="bg-white/20 p-4 rounded-3xl border border-white/20">
                    ...
                    >
                        {isLoggingOut ? (
                            ...
                            <>
                                ...
                                <Text className="text-red-500 font-black text-lg uppercase tracking-widest">Cerrar Sesión [V6_SOLUCION_FINAL]</Text>
                            </>
                        )}
                    </TouchableOpacity>
